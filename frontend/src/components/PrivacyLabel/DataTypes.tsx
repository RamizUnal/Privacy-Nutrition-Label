...
export default function DataTypes({ dataTypes }: Props) {
                  ...
                  <div className="mt-3 pt-3 border-t" style={{ borderColor: `${dt.color}20` }}>
                    ...
                    {dt.evidence.length > 0 && (
                      <div className="space-y-2">
                        <p className="text-xs font-mono text-white/30 uppercase tracking-wider">Evidence:</p>
                        {dt.evidence.map((ev, i) => (
                          <div key={i} className="p-2 rounded bg-black/30 border border-white/5">
                            <p className="text-xs font-mono text-white/40 leading-relaxed break-words">{ev}</p>
                          </div>
                        ))}
                      </div>
                    )}
